...
### Mapeo a actividad/subactividad del modelo de maquinaria (Captura_Diaria) — OBSOLETO desde D170

> `Modelo_Produccion_Maquinaria_v2` / `Captura_Diaria` dejaron de usarse (D170). El backend sigue derivando H/I y `a_captura` en la fila de MAQUINARIA para no romper lectores, pero ya no se pega nada. Se conserva como referencia del histórico.
...
**Regla de producción por tipo** (vigente; desde D170 se decide por CONTENIDO del `tipo` de `PARTE_EQUIPOS`: contiene COMPACTADOR / MINICARGADOR / MINIBULDOZER / RETROEXCAVADORA / RETROCARGADOR → sin producción, `esTipoSinProduccion`):
...
## 5. Motivos / Estados — OBSOLETO en el formulario del capataz (D170)

> Desde D170 el capataz **no** captura motivo ni horas, así que la fila de MAQUINARIA lleva `motivo` y ESTADO vacíos y el mapeo de abajo ya no se genera. El motivo de un día sin operación lo captura el operador en el Parte Digital (`Taller` · `Disponible` · `Domingo/Festivo` + descripción, D165). Se conserva como referencia del histórico.
