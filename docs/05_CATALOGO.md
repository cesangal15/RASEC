...
**Flota vigente UF1-UF2 al 15-sep-2026 (24 equipos; la verdad está en la hoja, esta tabla es la foto):**
...
| VOLQUETA | VOL012 · VOL044 · VOL048 · VOL056 · VOL065 · VOL067 | propias (VOL010/VOL047/VOL054 a taller 5-sep; VOL031 29-ago; VOL039 fuera desde 7-sep — confirmado sep-2026: la flota activa son estas 6 volquetas) |
...
**Actividades del parte digital (D174; D178: el operador ve solo las 5 más usadas de su equipo/tipo y «Otra…» en texto libre con CC opcional; el PR se admite en km):** el operador elige la actividad en palabras de obra y el CC sale solo (ítem + proyecto por PR). **Ojo al editar `PARTE_ITEMS`/`PARTE_CC` en el Sheet:** si una celda `item` queda como número (`2.1`), el código la lee igual como «02.10» (D178), pero conviene mantener la columna en formato texto (`setupParte()` lo fija). **Operadores (`PARTE_OPERADORES`, D178):** variantes de la misma persona en `PARTE_OPERADORES_ALIAS` (`CodigoParte.gs`) — una variante nueva = una línea ahí o `activo=NO` en la hoja; `depurarOperadoresParte(true)` ordena la hoja. La tabla vive en la hoja `PARTE_ITEMS` (`tipo_equipo · item · actividad`), sembrada del histórico; **dueño: Jeisson**. Etiquetas actuales: 01.02 Demolición de estructuras · 02.01 Desmonte y limpieza · 02.03 Descapote · 02.05 Excavación (cargue de volquetas) · 02.06 Excavación de préstamo · 02.07 Terraplén · 02.08 Botadero / ZODME · 02.10 Cargue terraplen (100 m a 1 km) · 02.11 Cargue terraplen (más de 1 km) — los dos son transporte de material de excavación/préstamo; la diferencia es la DISTANCIA de acarreo, no el material · 02.12 ya no se usa para el muro: **el muro de suelo reforzado va a 05.04** (D221) · 03.01 Subbase granular · 03.03 Base granular / BTC · 04.01 Riego de imprimación / humectación · 05.04 Terraplén en MSR / muro de suelo reforzado (`3701.05.04` «Relleno para muros de tierra MSR») · 06.01 Excavación para drenajes (ODT) · 06.02 Relleno de drenajes (ODT) · 06.05 Tubería de concreto (ODT) · 07.01 Cunetas en concreto · I0408 Transporte de personal (solo turbo y camión: en vibros, excavadoras, motoniveladoras y tractocamiones era error de digitación y se quitó de la tabla) · 11.01 Señalización / PMT. Los demás ítems llevan la frase del operador o, si no había una clara, el nombre del catálogo (hojas UF1/UF2 del Excel).
...
**Salida sep-2026 (dueño):** **TI12** (luminaria propia) sin parte desde 1-sep — fuera de la flota; se dio de baja en `MAQUINAS`.
...
**Marca/modelo/valor-hora en `dim`:** obsoleto desde D171 — el modelo `Modelo_Produccion_Maquinaria_v2` / `Captura_Diaria` quedó fuera de uso (ver maestro §10 y D171); la producción por máquina se reparte por CC del parte.
...
| Capataz (web) | actividades, producción medida (subbase/BTC/MSR/desmonte), **solo el CÓDIGO de la máquina por actividad (D171)** | Subbase, base, MSR, desmonte |
| Parte Digital de Maquinaria (QR en cabina, D165) | horas, operador, CC y motivo por equipo | Horas, operador, CC y motivo de cada máquina |
...
## 7. Usuarios — CONFIRMADO (claves en la hoja `USUARIOS`, D108)
**Usuarios y roles vigentes (claves en la hoja `USUARIOS`; alta/baja = editar una fila; fuente: §4 del maestro v2.0 y `PROJECT_CONTEXT.md`).**

| Rol | Usuario(s) | Entra a |
|---|---|---|
| `admin` | admin | `menu.html` |
| `capataz` (tierras) | angel · alejo · robinson · albert (volvió de UF3, D145) | `seleccion-reporte.html` / `reporte-capataz.html` |
| `chequeadora` | maleja · mairy · maria · luzdary | `seleccion-reporte.html` → `reporte-chequeadora.html` |
| `encargado` | encargado | `encargado.html` |
| `residente` | residente | `residente.html` |
| `jefe` | jefe | `jefe.html` |
| `capataz_odt` / `capataz_odl` | mauricio · eduardo · enrique (ODT) · jairo (ODL); los cuatro con áreas ODT+ODL | `reporte-drenajes.html` |
| `residente_dren` | residente_dren | `residente-drenajes.html` |
| `digitadora` | digitadora | `digitadora.html` |
| `asistencia_plus` | jeisson | `seleccion-reporte.html` |
| `asistencia_plus_dren` | duvan | `seleccion-reporte.html` |
| `asistencia_plus_uf3` | residente_uf3 | `seleccion-reporte.html` |
| `asistencia_plus_tm2` | angie | `seleccion-reporte.html` |
| `parte_maquinaria` | (opcional; hoy sin usuario) | `revision-maquinaria.html` |
| Revisión del parte digital (D178) | `admin`, `residente` (tile en `residente.html`), `jeisson` (por usuario, `PARTE_USUARIOS_REVISAN`; dos tiles en `seleccion-reporte.html`), `encargado`, `parte_maquinaria` | `revision-maquinaria.html` + `parte.html` (formulario con selector de equipo) |
| Operadores de maquinaria | sin login | `parte.html?eq=<código>` por QR (la identidad es el equipo, D165) |

Los usuarios `capataz1-5` / `chequeadora1-3` fueron los de la **etapa V1** (login genérico); hoy cada persona tiene su usuario nominal (trazabilidad en `reporta`). `ariel` salió a UF3 (D84): su llave no se reutiliza y su histórico se conserva.
