# INFORME 4.01 — Base de datos real: diagnóstico, decisión (D180) y plan
...
**Estado (16-sep-2026): Fases 3 (Asistencias) y 4 (Obra) PORTADAS en el repo; falta corte.** `CodigoAsistencias.gs` → `worker/src/api/asistencias.js` + `worker/src/api/asistencias/{catalogos,areas,personas,lectura,escritura}.js`; `Codigo.gs` → `worker/src/api/obra.js` + `worker/src/api/obra/{areas,flota,tablero,reporte,data,lectura,maquinaria}.js`, con el login y la emisión del token en `worker/src/auth.js` (tabla `usuarios`) y los catálogos de obra en `worker/src/catalogos.js`. Mismo método que la Fase 2 (mismos nombres de función que el `.gs`, una transacción por escritura, `ON CONFLICT DO NOTHING`, sin lock global; `LockService` → `pg_advisory_xact_lock` por `(fecha, cuadrilla)` en asistencias y `(fecha, area)` en `enviarData`; `enviarData` deja de regenerar `id_registro`; `volquetas` con PK surrogate y dedupe por línea). Migración de esquema `worker/sql/002_fases_3_4.sql` y backfills `worker/sql/backfill_obra.js` / `backfill_asistencias.js` (más `backfill_lib.js` común). **Verde en el banco local** (`worker/pruebas/contrato_local.js`, PGlite + backfill del volcado real): el arnés de contrato de los tres módulos contra el Worker da 57 casos · 133 comprobaciones · 0 fallos, y en modo vm (los `.gs` reales) sigue en 57 · 166 · 0. En `wrangler.toml`: `BACKEND_OBRA` y `BACKEND_ASISTENCIAS` en `"sheets"` (producción sigue en Google); las rutas `/prueba/*` en `"db"`. **Pendientes antes del corte:** (a) volcado REAL de asistencias (`volcarAsistencias()`; hoy el backfill de asistencias solo se probó con un CSV sintético) y depurar sus duplicados históricos por persona/día; (b) `002_fases_3_4.sql` y los backfills aplicados contra Supabase; (c) el espejo BD→Sheet de DATA (`data_maestro`) para el copy-paste al Excel maestro; (d) el corte por módulo (`BACKEND_ASISTENCIAS="db"`, luego `BACKEND_OBRA="db"`, cada uno con su `wrangler deploy` y vuelta atrás de un comando). Con el login en el Worker (obra en `db`), los tokens ya emitidos siguen valiendo (mismo `AUTH_SECRETO`/`AUTH_V`).

**Estado (16-sep-2026): CORTE HECHO — los tres módulos en Supabase.** El dueño aplicó `002_fases_3_4.sql` y los backfills (volcados `2026-09-16_1156_obra` y `2026-09-16_1157_asistencias`), puso `BACKEND_OBRA` y `BACKEND_ASISTENCIAS` en `"db"` y desplegó. Antes del deploy, asistencia y bandeja de ayer y hoy tenían las mismas filas en Sheets y en Supabase. Después se verificó en vivo que login (lo emite el Worker), obra (máquinas, bandeja, consolidado, volquetas, cubicaje, drenajes, tramos, flota, producción de maquinaria), asistencias (roster, día, export) y parte contestan desde Supabase, con CORS a `tm2.galca.app` y el rechazo genérico sin token. Único fallo: el tablero público salía vacío. La fila SÍ estaba en Supabase, pero guardada con doble codificación: postgres.js aplica `JSON.stringify` a los parámetros `jsonb` y el código le pasaba la foto ya convertida a texto, así que quedaba un string JSON y `tableroLeer` (que solo aceptaba objetos) respondía `foto:null`. Pasaba también al publicar desde la pantalla; en el banco local (PGlite) no se veía. Arreglado en `worker/src/api/obra/tablero.js` (`$n::text::jsonb` al escribir y lectura tolerante de la fila ya guardada) y en el backfill (`casts`); vuelve a salir al desplegar, sin recargar datos. Ya no se usa Sheets en el día a día: el copy-paste A:S al maestro sale de la pantalla del jefe (D65) y los catálogos se editan en Supabase; el espejo BD→Sheet de DATA queda opcional. Pendientes: Hyperdrive (4.15), depurar duplicados históricos de asistencia, dos semanas de `log` sin `error` y archivar los Apps Script (solo lectura 3 meses).
