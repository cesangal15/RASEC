...
/* Getter de la producción de cada partida para el avance, en suelto (se pasa a
   compacto ÷ fc al sumar). La EXCAVACIÓN común se lee de la columna «VOLUMEN
   APROV. UF1» del DATOS (campo `apr`), tal cual — sin inventar. */
const AV_GET = { excavacion:d=>d.apr||0,
...
/* ---------------------------------------------------------- agregación */
function construir(wbProd, wbMaq){
  const dias = leerProduccion(wbProd);
  const PLAN = leerPlan(wbProd);
  const H = wbMaq ? leerHoras(wbMaq) : { partes:[], cc:[], corte:null, negativas:0, descartadas:0 };
  const FC = FC_DEFECTO;
  ...
  const REND = leerProyectado(wbProd);
  ...
  const pers = [...new Set(dias.map(d=>d.p))].sort();
  ...
  function bloque(p, dd, hP, suf){
    ...
    for (const act of ACTS){
      ...
      a[act] = { prod:+prod.toFixed(1), dias:nd, equipos:eq,
    ...
    if (hP){
      ...
      for (const act of ACTS){
        ...
        const grupos = null;
        const mh = +(suelto/g.horas).toFixed(2);
        ...
        m[act] = { nombre:NOMBRE[act], prod:Math.round(suelto), horas:+g.horas.toFixed(1),
  ...
  const per = pers.map(p => {
    ...
    a.noaprov = { prod:+(dd.reduce((s,d)=>s+d.nap,0)/FC).toFixed(1),
    ...
    const split = { apr:+(dd.reduce((s,d)=>s+d.apr,0)/FC).toFixed(1),
                    pre:+(dd.reduce((s,d)=>s+d.pre,0)/FC).toFixed(1),
                    nap:+(dd.reduce((s,d)=>s+d.nap,0)/FC).toFixed(1) };
    return { p, d:dd.map(({p:_,...q})=>q), a, split, m, uf:ufb };
  });

  /* Base oficial (hasta 15-ago) + producción diaria desde el 16-ago, en compacto.
     La excavación usa la COMÚN = total − préstamo − no aprov (UF1+UF2), no el total. */
  const desde16 = dias.filter(d => d.f >= BASE_CORTE);
  const avance = ACTS.map(act => ({ k:act, n:NOMBRE[act].replace(' / Base',''),
    eje:+(BASE_ACUM[act] + desde16.reduce((s,d)=>s+AV_GET[act](d),0)/FC).toFixed(1),
    plan:CONTRATO[act] }));
  /* El préstamo es partida propia del contrato, aparte de la excavación común. */
  avance.push({ k:'prestamo', n:'Excavación préstamo',
    eje:+(BASE_ACUM.prestamo + desde16.reduce((s,d)=>s+(d.pre||0),0)/FC).toFixed(1),
    plan:CONTRATO.prestamo });
  ...
  return { fc:FC, desde:DESDE, per, avance,
    ...
    fuente:'TM2_SUR_REPORTE_DIARIO_OBRA · hojas DATOS y CALCULOS',
    fuente_horas:'BASE MAQUINARIA (partes diarios) · horas por centro de coste',
    unidad:'m³ compactos (suelto ÷ 1,3)', unidad_mh:'m³ compactos por hora de operación',
    generado: new Date().toISOString().slice(0,16).replace('T',' ') };
}

const MOTOR = { construir, normCod, periodoDe };
 ...
 *   avance      = acumulado desde 2025-08 / plan del contrato
 *
 ...
 * máquina no se puede reconstruir:
 * se dice «—, pulsa Actualizar», igual que la utilización, en vez de enseñar un
 * número que no es.
 */
...
function cadena(sel_p){
  ...
  if(uf!=='Todo' && !sel_p.uf){
    nota.textContent='';
    const v=el('div','nota');
    v.innerHTML='Esta foto no trae la maquinaria desglosada por UF — pulsa «Actualizar» para verla.';
  ...
  ACT.forEach(a=>{
    ...
    let subU;
    if(!hayU){
      subU='sin standby en esta foto — pulsa «Actualizar»';
    } else {
    ...
    if(vel!=null){
      ...
      vc.append(el('div','s','sin días por máquina en esta foto — pulsa «Actualizar»'));
    }
...
function diaria(p){
  const vis=visibles();
  const vals=p.d.map(d=>vis.map(a=>valDia(d,a)));
  const max=Math.max(1,...vals.flat());
  ...
  const medias=vis.map((a,j)=>{
    let acc=0;
    return p.d.map((d,i)=>{ acc+=vals[i][j]; return acc/(i+1); });
  });
  ...
  // leyenda de clima + relación lluvia/producción
  const lc=document.getElementById('legClima');lc.innerHTML='';
  ...
  const tot=p.d.map((d,i)=>vals[i].reduce((s,v)=>s+v,0));
  const llu=[],sec=[];
  p.d.forEach((d,i)=>{if(d.t==='LLUVIAS')llu.push(tot[i]);else if(d.t==='SOLEADO')sec.push(tot[i]);});
  const med=v=>v.length?v.reduce((s,x)=>s+x,0)/v.length:0;
  if(llu.length>=3&&sec.length>=3){
    const s=el('div');s.style.color='var(--accent-txt)';
...
function planPeriodo(p){
  ...
  if(!hay){
    const nt=el('div','nota');
    nt.innerHTML='Este período no trae planificado en la hoja <b>CALCULOS</b>, así que sólo se ve lo ejecutado.';
    c.appendChild(nt);
  }
}
 ...
 * ORIGEN DE LOS DATOS
 *
 * El tablero abre con la última foto guardada, así que en sala funciona aunque
 * no haya red. «Actualizar» vuelve a leer los dos Excel y recalcula TODO desde
 * las filas crudas — por eso las correcciones de centro de coste que hace el
 * jefe a fin de mes entran solas, sin tocar el código.
 *
 * Dos caminos, porque el navegador no puede abrir una ruta de disco por su
 * cuenta: si hay enlaces configurados los descarga; si no los hay o el servidor
 * los bloquea (CORS), se eligen los archivos a mano. El resultado es el mismo.
 * =========================================================================*/
const LS_URL='tm2-enlaces', LS_DAT='tm2-datos';
const $=id=>document.getElementById(id);

function enlaces(){
  try{ return JSON.parse(localStorage.getItem(LS_URL)||'{}'); }catch(e){ return {}; }
}
function estado(txt,warn){
  const e=$('estado');
  e.innerHTML=txt;
...
function pintaEstado(){
  /* El renglón queda al mínimo (decisión del jefe): solo hasta qué fecha llega
     cada fuente. «Producción actualizada al X» = último día con producción en el
     Excel; «maquinaria al Y» = corte de los partes diarios, que suele ir un par de
     días por detrás. Sin fechas (una foto vieja sin ese dato) se dice de cuándo es. */
  const prod = TM2.corte_prod || ultimaProd();
  const maq  = TM2.corte_horas || '';
  let s;
  if(prod && maq) s='Producción actualizada al <b>'+esc(prod)+'</b> y maquinaria al <b>'+esc(maq)+'</b>';
  else if(prod)   s='Producción actualizada al <b>'+esc(prod)+'</b>';
  else            s='Datos de '+(TM2.generado?('<b>'+esc(TM2.generado)+'</b>'):'la copia incluida');
  estado(s,false);
  if(!$('btnAct')) return;        // solo lectura: los botones ni existen
  leeH('prod').then(h=>{
    const b=$('btnAct'); if(!b) return;
    if(h) b.title='Relee «'+h.name+'» y el de maquinaria sin volver a preguntar';
    else if(HAY_FS) b.title='Elige los dos Excel una vez; después basta este botón';
  }).catch(()=>{});
}
...
async function leerLibro(origen,hojas){
  let buf;
  if(origen instanceof File){ buf=await origen.arrayBuffer(); }
  else{
...
async function recalcular(oProd,oMaq){
  const t0=Date.now();
  estado('Leyendo los Excel…',false);
  const bA=$('btnAct'), bE=$('btnArch');
  if(bA) bA.disabled=true; if(bE) bE.disabled=true;
  try{
    const wp=await leerLibro(oProd,['DATOS','CALCULOS']);
    const wm=oMaq?await leerLibro(oMaq,['BASE MAQUINARIA']):null;
    const d=MOTOR.construir(wp,wm);
    d.leidoDe = (oProd instanceof File) ? oProd.name : 'enlace configurado';
    /* Fecha de modificación del propio Excel leído. Es la prueba de que
       «Actualizar» agarró el archivo de HOY y no una copia vieja recordada:
       si aquí sale una fecha atrasada, el problema es el archivo, no el motor. */
    d.leidoFecha = (oProd instanceof File && oProd.lastModified)
      ? new Date(oProd.lastModified).toISOString().slice(0,16).replace('T',' ')
      : '';
    TM2=d; guardaDatos(d);
    sel=TM2.per.length-1; abierto=null;
    pinta();
    const seg=((Date.now()-t0)/1000).toFixed(1);
    /* Se publica DESPUÉS de pintar: si el Apps Script falla, quien actualizó ya
       tiene sus datos en pantalla y solo le falta compartirlos. Se le dice, en
       vez de dejarle creer que todo el mundo los está viendo. */
    estado('Actualizado en '+seg+' s · '+TM2.per.length+' períodos · publicando…',false);
    try{
      await publicar(d);
      estado('Actualizado en '+seg+' s · '+TM2.per.length+' períodos · horas hasta <b>'+
        esc(TM2.corte_horas||'—')+'</b> · <b>publicado</b>: el jefe y los residentes ya lo ven',false);
    }catch(err){
      estado('Datos actualizados en tu pantalla, pero NO se pudieron publicar: '+esc(err.message)+
        '. Los demás siguen viendo la foto anterior. Vuelve a pulsar «Actualizar» '+
        'cuando haya conexión.',true);
    }
  }catch(err){
    estado('No se pudo actualizar: '+esc(err.message)+
      '. Prueba con «Elegir archivos» — el resultado es el mismo.',true);
  }finally{
    if(bA) bA.disabled=false; if(bE) bE.disabled=false;
  }
}


/* ===========================================================================
 * LA FOTO ES COMPARTIDA (D158)
 *
 * Antes, «Actualizar» recalculaba y guardaba en el localStorage DE ESE
 * NAVEGADOR. Servía a quien pulsaba el botón y a nadie más: el jefe y los
 * residentes seguían viendo la copia que venía dentro del archivo. Eso no es un
 * tablero, es una hoja de cálculo con más pasos.
 *
 * Ahora la foto vive en el Sheet, por el mismo camino que el resto del sistema
 * (Apps Script). Quien publica, publica para todos y en el momento.
 *
 * CÓMO ABRE LA PÁGINA, en este orden y sin esperar a nadie:
 *   1. pinta YA con lo que tenga a mano — la foto embebida o la última vista;
 *   2. pide la compartida en segundo plano y, si llega y es distinta, repinta.
 * Así nunca hay pantalla en blanco delante de una sala, ni siquiera si el Apps
 * Script tarda o no hay red.
 *
 * PUBLICAR es cosa de admin y jefe, y lo decide el SERVIDOR con el token
 * firmado (D109): escribir `rol: admin` en el navegador no sirve de nada.
 * =========================================================================*/
const API = GALCA_ENV.url.obra;   // entorno.js (D168): producción o prueba

/* El token de sesión (D109) va a mano en la única llamada que lo necesita.
   `auth.js` se carga desde D169 solo porque es el único sitio con la URL base
   de la API (Worker api.galca.app), de la que entorno.js arma `GALCA_ENV.url`;
   como el token ya va puesto, auth.js no lo duplica. Es el mismo token firmado
   que emite el login; sin él el servidor no deja PUBLICAR. Leer la foto no lo
   pide (D161): es público, y el Worker deja pasar `action=tablero` sin token. */
function token(){ try{ return localStorage.getItem('tm2_token')||''; }catch(e){ return ''; } }

/* Trae la foto publicada. Va SIN token: la lectura es pública (D161), porque el
   directivo que abre el enlace no tiene sesión que mandar. Nunca lanza: si algo
   falla, la página se queda con la que ya está pintada, que es siempre mejor
   que un hueco. */
async function fotoCompartida(){
  try{
    /* Manda el token SI lo tiene. La lectura es pública (D159) y el servidor ya
       no lo exige, pero mientras el Apps Script no esté redesplegado con ese
       cambio, el servidor viejo SÍ lo pide: sin token, rechazaba la lectura y la
       foto compartida no cargaba (se quedaba con la incrustada, «no se actualiza»).
       Con el token cuando existe, funciona contra el servidor viejo y el nuevo;
       el directivo sin token lo leerá igual en cuanto se redespliegue. */
    const t = token();
...
/* Al abrir: se pide en segundo plano y solo se repinta si trae algo MÁS NUEVO,
   comparando el sello de generación. Repintar por repintar movería la pantalla
   debajo de quien ya está mirando. */
async function sincronizar(){
  const f = await fotoCompartida();
  if(!f) return;
  if(TM2.generado && f.generado && f.generado <= TM2.generado) return;
  TM2 = f; guardaDatos(f);
  sel = TM2.per.length-1; abierto = null;
  pinta();
}

/* Publicar: sube la foto que está en pantalla. El token lo pega auth.js. */
async function publicar(d){
  const r = await fetch(API, { method:'POST',
    headers:{ 'Content-Type':'text/plain;charset=utf-8' },
    body: JSON.stringify({ action:'tablero_guardar', foto:d, token:token() }) });
  if(!r.ok) throw new Error('el servidor respondió '+r.status);
  const j = await r.json();
  if(!j || j.ok===false) throw new Error((j&&j.error)||'no se pudo publicar');
  return j.meta||{};
}

/* ---------------------------------------------------------------------------
 * ARCHIVOS RECORDADOS (File System Access API)
 *
 * Lo que se pedía era «darle a actualizar y que él busque», con los archivos
 * siempre en el mismo sitio. Un enlace de SharePoint NO sirve: `:x:` abre el
 * visor y devuelve HTML, y aunque devolviera el .xlsx el navegador bloquea la
 * respuesta por CORS —Microsoft no manda `Access-Control-Allow-Origin`—.
 *
 * La vía buena es al revés: se eligen los dos archivos UNA vez y el navegador
 * guarda el permiso. A partir de ahí «Actualizar» los relee del disco solo. Como
 * la carpeta de SharePoint está sincronizada con OneDrive, son los mismos
 * archivos y siempre al día.
 *
 * Chrome y Edge, servida por https. En lo demás cae al selector de siempre.
 * ------------------------------------------------------------------------ */
const HAY_FS = typeof window.showOpenFilePicker === 'function';
const DB='tm2-fs', ALM='handles';

function abreDB(){
  return new Promise((ok,err)=>{
    const q=indexedDB.open(DB,1);
    q.onupgradeneeded=()=>q.result.createObjectStore(ALM);
    q.onsuccess=()=>ok(q.result); q.onerror=()=>err(q.error);
  });
}
function guardaH(k,v){
  return abreDB().then(db=>new Promise((ok,err)=>{
    const t=db.transaction(ALM,'readwrite'); t.objectStore(ALM).put(v,k);
    t.oncomplete=ok; t.onerror=()=>err(t.error);
  })).catch(()=>{});
}
function leeH(k){
  return abreDB().then(db=>new Promise(ok=>{
    const r=db.transaction(ALM,'readonly').objectStore(ALM).get(k);
...
/* Elegir los dos archivos y recordarlos. */
async function elegirRecordando(){
  try{
    estado('Elige el Excel de PRODUCCIÓN (TM2_SUR_REPORTE_DIARIO_OBRA)…',false);
    const [hp]=await showOpenFilePicker({multiple:false,types:[{description:'Excel',
      accept:{'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':['.xlsx','.xlsm']}}]});
    estado('Ahora el Excel de MAQUINARIA (partes diarios)…',false);
    const [hm]=await showOpenFilePicker({multiple:false,types:[{description:'Excel',
      accept:{'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':['.xlsx','.xlsm']}}]});
    await guardaH('prod',hp); await guardaH('maq',hm);
    await recalcular(await hp.getFile(), await hm.getFile());
  }catch(e){
    if(e && e.name==='AbortError'){ pintaEstado(); return; }
    estado('No se pudieron abrir los archivos: '+esc(e.message),true);
  }
}
/* Releerlos sin preguntar nada. */
async function releerRecordados(pedir){
  const hp=await leeH('prod'); if(!hp) return false;
  const hm=await leeH('maq');
  if(!await permiso(hp,pedir)) return false;
  if(hm && !await permiso(hm,pedir)) return false;
  await recalcular(await hp.getFile(), hm?await hm.getFile():null);
  return true;
}

/* Selector clásico, para navegadores sin la API de arriba. */
function pedirArchivos(){
  const fp=$('filProd'), fm=$('filMaq');
  fp.value=''; fm.value='';
  fp.onchange=()=>{
    if(!fp.files[0]) return;
    estado('Ahora elige el Excel de maquinaria (partes diarios)…',false);
    fm.onchange=()=>recalcular(fp.files[0], fm.files[0]||null);
    fm.click();
  };
  estado('Elige el Excel de producción (TM2_SUR_REPORTE_DIARIO_OBRA)…',false);
  fp.click();
...
/* ===========================================================================
 * QUIÉN ENTRA Y QUÉ PUEDE TOCAR (D158, abierta al público en D161)
 *
 ...
 * la ven, sin login y sin instalar nada. Quien SÍ entra por la plataforma la
 * sigue teniendo en su panel —jefe.html, residente.html, seleccion-reporte.html—
 * y además le sale el botón de volver, que el visitante público no necesita.
 *
 * QUÉ QUEDA A LA VISTA: producción, horas de máquina y avance. Ni nombres de
 * personas, ni usuarios, ni claves — la foto no los lleva. Las CIFRAS DE OBRA
 * sí las ve cualquiera que tenga el enlace: es la decisión que se tomó, no un
 * descuido.
 *
 * EL PORTERO DE ANTES SE FUE, Y NO PROTEGÍA NADA. Los datos van EMBEBIDOS en
 * este archivo: quien tiene la URL tiene los números, con sesión o sin ella.
 * Aquel desvío a index.html era de cortesía y ahora estorbaba. Lo que sí se
 * guarda de verdad es ESCRIBIR, y eso lo decide el SERVIDOR con el token
 * firmado (D109), no este archivo.
 *
 * QUIÉN PUEDE ACTUALIZAR. Los botones de «Actualizar» y «Elegir archivos»
 * releen los dos Excel y publican la foto PARA TODOS. Los tienen el ADMIN y el
 * JEFE, que son los dos que manejan esos archivos; los residentes y el público
 * no, porque quien elige el Excel equivocado antes de una reunión ve cifras
 * que no son y no tiene cómo notarlo. El resto ve la foto publicada y el
 * renglón que dice de cuándo es.
 * =========================================================================*/
...
function segunRol(rol){
  ...
  /* Se retiran los tres botones de la barra de datos y el panel de enlaces.
     El renglón de estado se queda: dice de cuándo es la foto, que es
     justamente lo que hay que saber antes de proyectarla. */
  ['btnAct','btnArch','btnCfg'].forEach(id=>{ const b=$(id); if(b) b.remove(); });
  const c=$('cfg'); if(c) c.remove();
}

/* ------------------------------------------------------------- arranque */
function wire(){
  if(!$('btnCfg')) return;        // solo lectura: no hay nada que cablear
  const u=enlaces();
  $('urlProd').value=u.prod||''; $('urlMaq').value=u.maq||'';
  $('btnCfg').onclick=()=>{ $('cfg').hidden=!$('cfg').hidden; };
  $('btnGuardar').onclick=()=>{
    const v={prod:$('urlProd').value.trim(),maq:$('urlMaq').value.trim()};
    try{ localStorage.setItem(LS_URL,JSON.stringify(v)); }catch(e){}
    $('cfg').hidden=true;
    estado(v.prod?'Enlaces guardados. Pulsa «Actualizar».'
                 :'Enlaces borrados: «Actualizar» pedirá los archivos.',false);
  };
  $('btnArch').onclick=()=>HAY_FS?elegirRecordando():pedirArchivos();
  $('btnAct').onclick=async ()=>{
    if(HAY_FS && await releerRecordados(true)) return;   // los de siempre, un clic
    const v=enlaces();
    if(v.prod){ recalcular(v.prod, v.maq||null); return; }
    estado('Aún no hay archivos recordados — elígelos una vez y ya no vuelvo a preguntar.',true);
    HAY_FS?elegirRecordando():pedirArchivos();
  };
}
...
try{const s=localStorage.getItem(LS_DAT);
    if(s){const d=JSON.parse(s); if(d&&d.per&&d.per.length) TM2=d;}}catch(e){}
...
pinta();
logCC();
sincronizar();            // en segundo plano; si no hay red, no pasa nada
