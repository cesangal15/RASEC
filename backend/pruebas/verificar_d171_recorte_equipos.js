#!/usr/bin/env node
/**
 * Verificación D171 — Recorte de equipos en el reporte del capataz: solo el CÓDIGO de la máquina.
 *
 ...
 *   8 · Frontend: reporte-capataz.js sin horas/operador/motivo (manejadores `data-on-*`, CSP sin inline de D170);
 *       flota.js con `equiposCapataz` (y su caída a `maquinas`); encargado.js sin horas; estado.html/menu.html
 *       marcados obsoletos sin estilos en línea; sw.js con CACHE_V nuevo.
 *
 *   node backend/pruebas/verificar_d171_recorte_equipos.js
 */
...
{
  const cap=leer('reporte-capataz.js');
  ok('reporte-capataz.js sin horas/operador/motivo (e-hrs, e-op, e-mot-sel, MOTIVOS, gatherMachineGroups, progHoras)', !/e-hrs|e-op\b|e-mot-sel|const MOTIVOS|gatherMachineGroups|progHoras|horas_operadas/.test(cap));
  ok('…sin manejadores ni estilos en línea (CSP de D170): solo data-on-* / clases', !/ on(click|input|change)=| style="/.test(cap) && /data-on-click="togglePicker\(/.test(cap) && /data-on-input="renderPicker\(/.test(cap));
  ok('…el marcado (reporte-capataz.html) no tiene <script> en línea y carga reporte-capataz.js', !/<script>/.test(leer('reporte-capataz.html')) && /reporte-capataz\.js/.test(leer('reporte-capataz.html')));
  ...
  return F.cargar('http://x', HOY, { ids:['BL005'], tipos:{BL005:'BULLDOZER'}, prog:{} }).then(fl=>{
    ...
    ok('encargado.js: bandeja y WhatsApp de equipos sin horas ni operador', !/maqResumen|horas_operadas|s\/op/.test(enc) && /D171/.test(enc));
    ok('estado.html: aviso de obsoleto que remite a revision-maquinaria.html, sin style= en línea', /obsoleta \(D171\)/.test(leer('estado.html')) && /revision-maquinaria\.html/.test(leer('estado.html')) && !/aviso-obsoleto"[^>]*style=/.test(leer('estado.html')) && /\.aviso-obsoleto/.test(leer('estado.css')));
    ok('menu.html: «Estado maquinaria (capataz, obsoleto)» sin style= en línea', /Estado maquinaria \(capataz, obsoleto\)/.test(leer('menu.html')) && !/tile-obsoleto" href="estado.html" style=/.test(leer('menu.html')) && /\.tile-obsoleto/.test(leer('menu.css')));
    ok('sw.js: CACHE_V subido (flota.js cambió y el formulario nuevo depende de él)', /CACHE_V = 'tm2-v12'/.test(leer('sw.js')));
